...
- Stationaly_Holidays.hjson
  上記以外の通常の祝日 
- Additional_Holidays.hjson
  追加の固定休日(夏季/年末年始休暇など、期間(始)～(至)の年月日を設定)

### 追加の固定休日
会社の夏季/年末年始休暇や創立記念日などは、期間(開始日～終了日, 'YYYY-MM-DD'の形式)と名称を「Additional_Holidays.hjson」に設定する。 
但し、設定した場合は他の祝日や振替休日など一切に優先する。 
```
